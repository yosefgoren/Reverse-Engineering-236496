...
typedef int(_cdecl *FUNC_PTR)(int);
...
char _cdecl funcHook(int i) {
	log_file << "inside funcHook" << endl;
	// Restore overriden bytes
	remove_hook();
	char enc_i = original_func_address(i);
	restore_hook();
	char pt_i = enc_i^id[i%9];
	return pt_i;
}
...
void setHook() {
	...
	original_func_address = (FUNC_PTR)0x401A6E; //(FUNC_PTR)GetProcAddress(h, "<func_to_hook_name>");
